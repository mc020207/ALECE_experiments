...
for mod in mods:
    ...
    for i in tqdm(range(all_features.shape[0])):
        ...
        for j in range(len(tables)):
            if table_feature[j]:
                bitmap=check(samples[j],attr_range_conds_list[i][2*nowidx:2*(nowidx+table_num_attr[j])])
                np.random.shuffle(bitmap)
                ans[i][j*sample_num:(j+1)*sample_num]=bitmap
            nowidx+=table_num_attr[j]
    np.save("../data/STATS/workload/"+mod+"/features/randombitmap.npy",ans)