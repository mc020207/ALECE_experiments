...
import time
def train_with_batch(model, args, train_data, validation_data, curr_ckpt_step, test_data, q_error_dir):
    ...
    np.save(os.path.join(args.experiments_dir,(args.wl_type+"_X_value.npy")),X_value)
    np.save(os.path.join(args.experiments_dir,(args.wl_type+"_Y_value.npy")),Y_value)
    plt.xlabel('epochs')
    plt.ylabel('eval_loss')
    plt.savefig(os.path.join(args.experiments_dir,(args.wl_type+"_plt.png")))
...
def load_data(args):
    ...
    np.save(os.path.join(args.experiments_dir,(args.wl_type+"_validation_idx.npy")),shuffle_idxes[N_train:])
...
def pred_record(model,args,test_data,q_error_dir,epoch):
    preds = eval(model, args, test_data, q_error_dir)

    # ====================================================
    preds = preds.tolist()
    workload_dir = arg_parser_utils.get_workload_dir(args, test_wl_type)
    experiments_dir=os.path.join(args.experiments_dir,("epoch_"+str(epoch)))
    e2e_dir = os.path.join(experiments_dir, args.e2e_dirname)
...
if __name__ == '__main__':
    ...
    if training:
        train_with_batch(model, args, train_data, validation_data, ckpt_step, test_data, q_error_dir)
